...
def generating_bid_value(model, origin_data, avgCTR, base_bid, start_budget = 25000000):  
    impressions = 0
    clicks = 0
    budget=start_budget
    STD_SLOTPRICE = origin_data.slotprice.values.std()
...
def generating_bid_value_for_test(model, origin_data, avgCTR, base_bid): 
    STD_SLOTPRICE = origin_data.slotprice.values.std()
...
for base_bid in Bid['Base_Bid']:
    [imps, clicks, cost] = generating_bid_value(model, validation_data, avgCTR, base_bid)
    im.append(imps)
    clks.append(clicks)
    ct.append(cost) 

    [bid_id, bid_price] = generating_bid_value_for_test(model, test_data, avgCTR, base_bid)
Bid['Impressions'] = im
Bid.Impressions = Bid.Impressions.astype(int)
Bid['Spend'] = ct
Bid['Clicks'] = clks
Bid['CTR'] = (Bid.Clicks/Bid.Impressions * 100).round(4).astype(str)
